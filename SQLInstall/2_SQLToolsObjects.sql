--This script creates the SQLTools database as well as all of the objects associated with it as part of a new server build
...
GO
ALTER DATABASE [SQLTools] SET RECOVERY SIMPLE
GO
...
/****** Object:  StoredProcedure [dbo].[CommandExecute]    Script Date: 11/10/2015 3:46:48 PM ******/
SET ANSI_NULLS ON
GO
...
END
GO
/****** Object:  StoredProcedure [dbo].[DatabaseIntegrityCheck]    Script Date: 11/10/2015 3:37:37 PM ******/
...
GO
/****** Object:  Table [dbo].[Connections]    Script Date: 11/10/2015 3:37:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  Table [dbo].[IndexDisable]    Script Date: 11/10/2015 3:37:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[IndexDisable](
 ...
 CONSTRAINT [IndexDisable_PK] PRIMARY KEY CLUSTERED 
(
	[database_id] ASC,
	[schema_id] ASC,
	[object_id] ASC,
...
GO
/****** Object:  Table [dbo].[IndexTally]    Script Date: 11/10/2015 3:37:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
CREATE TABLE [dbo].[IndexTally](
	[database_id] [smallint] NOT NULL,
	[object_id] [int] NOT NULL,
	[index_id] [int] NOT NULL,
	...
	[fragmentMB] [numeric](29, 7) NULL,
	[db_name] [nvarchar](60) NOT NULL,
	[run_timestamp] [datetime] NOT NULL,
	[ignore_index] [bit] NULL,
	[online_reorg_possible] [bit] NULL,
 CONSTRAINT [IndexTally_PK] PRIMARY KEY CLUSTERED 
(
	[database_id] ASC,
	[object_id] ASC,
	[index_id] ASC,
	[partition_number] ASC,
	[alloc_unit_type_desc] ASC,
	[run_timestamp] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, FILLFACTOR = 90)) 
...
GO
/****** Object:  StoredProcedure [dbo].[IndexOptimize]    Script Date: 11/10/2015 3:37:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
GO
/****** Object:  StoredProcedure [dbo].[up_TrackConnections]    Script Date: 11/10/2015 3:37:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE PROCEDURE [dbo].[up_TrackConnections]
(@db nvarchar(128) = NULL)
AS BEGIN
  SET NOCOUNT ON;

IF @db IS NULL
INSERT INTO SQLTools.dbo.Connections (dbname, hostname, loginname, Program, logintime)
    SELECT DISTINCT
      RTRIM(DB_NAME(sp.dbid))
    , RTRIM(sp.hostname)
    , RTRIM(sp.loginame)
	, RTRIM(sp.program_name)
    , getdate()
    FROM master.dbo.sysprocesses sp
    WITH (NOLOCK)
    WHERE NOT EXISTS (SELECT *
                      FROM SQLTools.dbo.Connections cn
                      WHERE DB_NAME(sp.dbid) = cn.dbname 
                        AND sp.hostname = cn.hostname
                        AND sp.loginame = cn.loginname
						AND sp.program_name = cn.Program)
UPDATE cn 
SET cn.logintime = GETDATE()
FROM SQLTools.dbo.Connections cn
JOIN master.dbo.sysprocesses sp ON DB_NAME(sp.dbid) = cn.dbname
	AND sp.hostname = cn.hostname
	AND sp.loginame = cn.loginname
	AND sp.program_name = cn.Program
  RETURN 0;
...
/****** Object:  StoredProcedure [dbo].[usp_DBA_ShrinkTlog]    Script Date: 12/31/2015 9:22:12 AM ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
-- Author:		RONALD NOMA IGBINOMWANHIA
-- Modified date: 12/31/2014
-- Description:	SHRINK LOG FILES
-- Deployed 1/8/2015 
-- =============================================
CREATE PROCEDURE [dbo].[usp_DBA_ShrinkTlog] 
...
SET NOCOUNT ON
/**TEMP TABLE TO STORE LOG PATH TO LOG FILES TO BE SHRUNK**/
CREATE TABLE #LogFiles (Dbname NVARCHAR(150), LogFile NVARCHAR(150) )

/**CREATE TEMP TABLE TO HOLD DATABASE_ID WHIHCH WOULD BE JOINED TO SYS.DATABASES TO GENERATE NAEMS OF DATABASES NOT INVOLVED IN BACKUP OR RESTORE**/
CREATE TABLE #Output (database_id NVARCHAR(15));


/**CREATE TEMP TABLE TO HOLD S/N (to be used as count for the while loop) and DATABASE_ID **/


CREATE TABLE #CheckBackup (sn INT ,database_id NVARCHAR(15))
INSERT INTO #CheckBackup 

SELECT  ROW_NUMBER() OVER (ORDER BY database_id) AS 'sn', database_id
FROM	sys.databases 
WHERE	name NOT IN ('master','tempdb','model','msdb','distribution') AND  
		name NOT LIKE 'ReportServer%' AND is_read_only = '0' AND state_desc != 'OFFLINE'
		
--declare varibles
DECLARE @count	INT
DECLARE @iteration	INT
--initialize variables
	SELECT @count =  MAX(sn) FROM #CheckBackup
	SELECT @iteration  = MIN (sn) FROM #CheckBackup

		WHILE @iteration <= @count
			BEGIN
			DECLARE @Dbid NVARCHAR(15)
			SELECT @Dbid = database_id FROM #CheckBackup
			WHERE sn = @iteration
	--check if database is currently being backedup\restored.

				IF NOT EXISTS (
						SELECT database_id FROM sys.dm_exec_requests
						WHERE command  LIKE '%RESTORE%' OR command NOT LIKE '%BACKUP%'AND database_id != 1
						AND database_id = @Dbid)


--print @Dbid   --troubleshooting

--insert output of database_ids not involved in backup/restore into #Output temporary table
					INSERT INTO #Output VALUES (@Dbid) 
	...
	/*-- step 2. insert all the database name and corresponding log files' names into the temp table*/
	 ...
	 DECLARE ListDb CURSOR FAST_FORWARD FOR
	 SELECT   name 
		FROM sys.databases sd
		INNER JOIN #Output tp ON sd.database_id = tp.database_id
		...
		OPEN ListDb FETCH
		NEXT FROM ListDb INTO @Db
		WHILE @@FETCH_STATUS <> -1 
		BEGIN
		SET NOCOUNT ON
		
		SET @Sql = 'USE [' + @Db + ']; INSERT INTO #LogFiles(Dbname, LogFile) SELECT '''
		+ @Db + ''', Name FROM sysfiles WHERE FilENAME LIKE ' + '''%.ldf'''
		--SELECT @SqlScript
		---PRINT @Sql
		EXEC  (@Sql)
		FETCH NEXT FROM ListDb INTO @Db END
		CLOSE ListDb
		DEALLOCATE ListDb;

		/*cursor to shrink log files*/

		DECLARE listlog CURSOR FOR 
		SELECT Dbname, LogFile FROM #LogFiles 
		DECLARE @LogFile NVARCHAR(128) 

		OPEN listlog FETCH
		NEXT FROM listlog INTO @Db, @LogFile
		WHILE @@FETCH_STATUS <> -1 
		BEGIN 
		SELECT @Sql = 'USE [' + @Db + ']; '
		...
		EXEC (@Sql)
		--PRINT @Sql
		FETCH NEXT FROM listlog INTO @Db, @LogFile END
		DEALLOCATE listlog;

		--select * from #LogFiles
		DROP TABLE #LogFiles;
		DROP TABLE #CheckBackup;
		DROP TABLE #Output;
	
		END
GO

/****** Object:  Table [dbo].[CommandLog]    Script Date: 11/10/2015 3:37:37 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
...
/****** Object:  UserDefinedFunction [dbo].[DatabaseSelect]    Script Date: 11/11/2015 4:16:52 PM ******/
SET ANSI_NULLS ON
GO
